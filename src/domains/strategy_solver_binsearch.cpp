...
bool strategy_solver_binsearcht::iterate(invariantt &_inv)
{
...
#endif
    tpolyhedra_domain.strategy_cond_literals[i]=solver.convert(strategy_cond_exprs[i]);
    // solver.set_frozen(tpolyhedra_domain.strategy_cond_literals[i]);
    strategy_cond_exprs[i]=literal_exprt(tpolyhedra_domain.strategy_cond_literals[i]);
  }
#if 0
  debug() << eom;
#endif
  ...
  if(solver()==decision_proceduret::D_SATISFIABLE) // improvement check
  {
    ...
    for(std::size_t i=0; i<tpolyhedra_domain.template_size(); i++)
    {
      ...
      debug() << "expr: " << from_expr(ns, "", tpolyhedra_domain.strategy_value_exprs[i]) << " "
              << from_expr(
                ns, "", simplify_const(solver.get(tpolyhedra_domain.strategy_value_exprs[i])))
              << eom;
      debug() << "guards: "
