  ...
  const local_SSAt &SSA)
{
          ...
          options.get_bool_option("heap-zones"))
  {
    ...
    tpolyhedra_domaint *phd=
      static_cast<tpolyhedra_domaint*>(tmp_ptr->domains[1]);

    if(polyhedra_kind==heap_tpolyhedra_sympath_domaint::INTERVAL)
      phd->add_interval_template(var_specs, SSA.ns);
    else if(polyhedra_kind==heap_tpolyhedra_sympath_domaint::ZONES)
    {
      phd->add_difference_template(var_specs, SSA.ns);
      phd->add_interval_template(var_specs, SSA.ns);
    }
    }
  }
